...
async function updateData({page, sortBy}) {
  ...
  const res = await fetch("http://192.168.255.6:8080/v1/dataset?" + new URLSearchParams({
...
async function updateAttribute({page, sortBy}) {
  ...
  const res = await fetch(`http://192.168.255.6:8080/v1/dataset/${currentDatasetId.value}/attributes?` + new URLSearchParams({
...
<template>
  ...
  <template v-slot:thead>
    <tr>
      <td></td>
      <td>
        <v-text-field v-model="search" class="ma-0" density="compact" placeholder="Search name..." hide-details></v-text-field>
      </td>
    </tr>
  </template>
...
><v-card><v-data-table-server
  ...
  <template v-slot:thead>
    <tr>
      <td>
        <v-text-field v-model="search2" class="ma-0" density="compact" placeholder="Search name..." hide-details></v-text-field>
      </td>
    </tr>
  </template>
